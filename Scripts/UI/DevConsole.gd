...
func _input(event: InputEvent) -> void:
	...
	if is_open and event.is_action_pressed("ToggleConsole"):
		toggle_console()
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if not is_open and event.is_action_pressed("ToggleConsole"):
		toggle_console()
		get_viewport().set_input_as_handled()
		return
...
func _register_commands() -> void:
	...
	registry.register_command("set_room", ConsoleCommands.cmd_set_room, "Change la room actuelle", 1, 2, "set_room <room> [spawnpoint]")
